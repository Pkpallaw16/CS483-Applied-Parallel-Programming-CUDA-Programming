...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if (x < dim) {
    float dot_prod = 0.0f;
    for (int element = 0; element < matRows[x]; element++) {
      int idx = matColStart[element] + x;
      dot_prod += matData[idx] * vec[matCols[idx]];
    }
    // Store the result in the output vector with the correct permutation
    out[matRowPerm[x]] = dot_prod;
  }
}
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  dim3 gridDim((dim + TILE_WIDTH - 1) / TILE_WIDTH); 
  // Launch the kernel
