    ...
    for (int m = 0; m < Map_out; m++) {
        for (int h = 0; h < Height_out; h++) {
            for (int w = 0; w < Width_out; w++) {
                ...
                // Initialize output value for each (b, m, h, w) position
                float output_val = 0.0f;

                // Iterate through input channels and the KxK convolution kernel
                for (int c = 0; c < Channel; c++) {
                    for (int p = 0; p < K; p++) {
                        for (int q = 0; q < K; q++) {
                            // Calculate input and mask values separately
                            float input_val = in_4d(b, c, h + p, w + q);
                            float mask_val = mask_4d(m, c, p, q);

                            // Accumulate the result
                            output_val += input_val * mask_val;
                        }
                    }
                }

                // Store the accumulated result in the output
                out_4d(b, m, h, w) = output_val;
            }
        }
    }
}
