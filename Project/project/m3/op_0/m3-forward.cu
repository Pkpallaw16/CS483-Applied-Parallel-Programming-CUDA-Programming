...
__constant__ float constant_mask[4000]; // Adjust the size based on maximum Map_out * Channel * K * K
...
__global__ void matrixMultiplyShared(const float *A, const float *B, float *C,
    ...
    for (int tileId = 0; tileId < (numAColumns - 1) / TILE_WIDTH + 1; tileId++) {
        if (row < numARows && tileId * TILE_WIDTH + tx < numAColumns) {
            tileA[ty][tx] = constant_mask[row * numAColumns + tileId * TILE_WIDTH + tx];
        } else {
            tileA[ty][tx] = 0;
        }
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input,
    ...
    cudaMemcpyToSymbol(constant_mask, host_mask, mask_size);
}
