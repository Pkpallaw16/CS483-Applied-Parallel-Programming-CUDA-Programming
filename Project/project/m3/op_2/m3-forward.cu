...
__global__ void matrix_unrolling_kernel(const float *input, float *output,
    ...
    */
    ...
    if (h_out < H_out && w_out < W_out && b < Batch) {
        ...
        // If Channel, K are known and small, we can fully unroll
        // #pragma unroll can be applied to loops with known constants.
                                     ...
                                     int numCRows, int numCColumns)
{
    ...
    int tile_count = (numAColumns - 1) / TILE_WIDTH + 1;
    ...
    for (int tileId = 0; tileId < tile_count; tileId++) {
        ...
        if (row < numCRows && col < numCColumns) {
            // Unroll this loop since TILE_WIDTH is a compile-time constant (e.g., 16)
            #pragma unroll
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input,
    ...
    // Adjust grid dimensions for matrix multiplication
